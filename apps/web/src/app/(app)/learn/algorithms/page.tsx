...
function SectionHeader({ children }: { children: React.ReactNode }) {
    return (
        <h3 className="border-border/60 text-muted-foreground mt-2 border-b pt-2 pb-3 text-xs font-semibold tracking-wider uppercase">
            {children}
        </h3>
    );
}
...
function Section({
    ...
    return (
        <Card className={nested ? 'border-border/60' : undefined}>
            <CardHeader className="space-y-3 pt-5 pb-4">
                <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                        <span
                            className={cn(
                                'inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full font-mono text-xs font-semibold',
                                nested
...
function StatusPill({ status }: { status: Status }) {
    ...
    return (
        <span
            className={cn(
                'inline-flex shrink-0 items-center rounded-full border px-2 py-0.5 text-[10px] font-medium tracking-wide uppercase',
                styles[status],
            )}
        >
            {label[status]}
